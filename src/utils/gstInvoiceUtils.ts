        ...
        <div class="grid2">
          <div class="box">
            <div style="font-weight: bold; font-size: 13px; margin-bottom: 5px;">
              ${businessProfile?.displayName || 'VAMS Veterinary Consultancy Private Limited'}
            </div>
...
export const printGSTInvoice = (invoice: Invoice, customer: Customer, businessProfile?: BusinessProfile) => {
  const printWindow = window.open('', '_blank', 'width=800,height=600');
  if (!printWindow) return;

  const invoiceHTML = generateGSTInvoiceHTML(invoice, customer, businessProfile);
...
export const downloadGSTInvoice = async (invoice: Invoice, customer: Customer, businessProfile?: BusinessProfile) => {
  try {
    ...
    tempContainer.innerHTML = generateGSTInvoiceHTML(invoice, customer, businessProfile);
    ...
    // Download the PDF
    const fileName = `GST-Invoice-${invoice.invoice_number || invoice.invoice_id || invoice.id}.pdf`;
    pdf.save(fileName);
    ...
    link.href = url;
    link.download = `GST-Invoice-${invoice.invoice_number || invoice.invoice_id || invoice.id}.html`;
